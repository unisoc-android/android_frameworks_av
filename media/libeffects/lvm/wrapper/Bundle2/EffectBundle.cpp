...
#endif
#define LOG_TAG "audio_hw_Bundle"
...
#define SPRD_AUD_DSP_EFFECT "/dev/audio_pipe_effect"

#define EFFECT_CMD_OPEN 100
#define EFFECT_CMD_CLOSE 101

#define COMMON_COMMAND_EFFECT 0

static int g_file_hanle = 0;

typedef struct effect_command {
    uint32_t common_command;
    uint32_t effect_type;
    uint32_t effect_cmd_code;
    char cmd_data[52];
} effect_command;

typedef struct effect_reply {
    uint32_t common_command;
    uint32_t effect_type;
    uint32_t effect_cmd_code;
    char reply_data[52];
...
#if 0
static int get_reply_from_DSP(void *reply_data)
{
    int ret;
    effect_reply reply;

    ALOGE("%s,g_file_hanle=%d, reply_data=%p",
        __func__, g_file_hanle, reply_data);

    if (g_file_hanle <= 0  || reply_data == 0)
        return 0;

    ret = read(g_file_hanle, &reply, sizeof(reply));
    ALOGE("%s, ret=%d", __func__, ret);

    if (ret < 0)
        return ret;

    memcpy(reply_data, (void *)&reply.reply_data, 52);
    return 0;
}
#endif

static int send_command_to_DSP(uint32_t effect_type,
                                        uint32_t cmd_code,
                                        uint32_t cmd_size,
                                        void *cmd_data )
{
...
//    effect_reply reply;
    ...
    if (g_file_hanle <= 0)
        g_file_hanle = open(SPRD_AUD_DSP_EFFECT, O_RDWR);

    ALOGD("%s,g_file_hanle=%d, effect_type=%d,cmd_code=%d,cmd_size=%d,cmd_data=%p",
            __func__, g_file_hanle, effect_type, cmd_code, cmd_size, cmd_data);

    msg.common_command = COMMON_COMMAND_EFFECT;
    msg.effect_type = effect_type;
    msg.effect_cmd_code = cmd_code;

    if (cmd_size > 0)
        memcpy((void *)msg.cmd_data, cmd_data, cmd_size);
    ret = write(g_file_hanle, &msg, sizeof(msg));

    ALOGD("%s, write ret=%d", __func__, ret);
#if 0
    if (ret < 0)
        return ret;
    ret = read(g_file_hanle, &reply, sizeof(reply));

    ALOGD("%s, read ret=%d", __func__, ret);
#endif
    return ret;
}
...
const effect_descriptor_t gBassBoostDescriptor = {
        ...
        | EFFECT_FLAG_VOLUME_CTRL | EFFECT_FLAG_OFFLOAD_SUPPORTED),
...
const effect_descriptor_t gEqualizerDescriptor = {
        ...
        EFFECT_CONTROL_API_VERSION,
        (EFFECT_FLAG_TYPE_INSERT | EFFECT_FLAG_INSERT_FIRST | EFFECT_FLAG_VOLUME_CTRL | EFFECT_FLAG_OFFLOAD_SUPPORTED),
...
const effect_descriptor_t gVolumeDescriptor = {
        ...
        EFFECT_CONTROL_API_VERSION,
        (EFFECT_FLAG_TYPE_INSERT | EFFECT_FLAG_INSERT_LAST | EFFECT_FLAG_VOLUME_CTRL | EFFECT_FLAG_OFFLOAD_SUPPORTED),
...
extern "C" int EffectCreate(const effect_uuid_t *uuid,
    ...
    pContext->offload_en = 0;
#if 0
    send_command_to_DSP(pContext->EffectType,
                        EFFECT_CMD_OPEN,
                        0,
                        NULL);
#endif
...
int Volume_setParameter(EffectContext *pContext,
    ...
    switch (params[0]) {
        case VOLUME_PARAM_LEVEL: {
            ...
            ALOGD("%s VOLUME_PARAM_LEVEL %d", __func__, level);
            ALOGD("%s VOLUME_PARAM_LEVEL Calling VolumeSetVolumeLevel", __func__);
            status = VolumeSetVolumeLevel(pContext, level);
            ALOGD("%s VOLUME_PARAM_LEVEL Called VolumeSetVolumeLevel", __func__);
        } break;
...
//    pContext->pBundledContext->SamplesToExitCountEq);
    ...
    if(pContext->pBundledContext->NumberEffectsCalled ==
       pContext->pBundledContext->NumberEffectsEnabled){
        ...
        ALOGV("effect_process:type=%d", pContext->EffectType);
    } else {
        ...
        ALOGV("effect_process:type=%d, bypassed", pContext->EffectType);
    }
...
int Effect_command(effect_handle_t  self,
    ...
    //        pContext->pBundledContext->NumberEffectsCalled,
    //        pContext->pBundledContext->NumberEffectsEnabled);
    ALOGD("Effect_command:%d",cmdCode);

    switch (cmdCode){
        case EFFECT_CMD_OFFLOAD:
            if (cmdSize != sizeof(effect_offload_param_t) || pCmdData == NULL
                    || pReplyData == NULL || *replySize != sizeof(int)) {
                ALOGD("%s EFFECT_CMD_OFFLOAD bad format", __func__);
                return -EINVAL;
            }
            {
                effect_offload_param_t* offload_param = (effect_offload_param_t*)pCmdData;

                pContext->offload_en = offload_param->isOffload;
                *(int *)pReplyData = 0;

                ALOGD("%s EFFECT_CMD_OFFLOAD offload_en=%d",
                      __func__, pContext->offload_en);
            }
            break;
            ...
            if(pContext->EffectType == LVM_EQUALIZER){
                ...
                *(int *)pReplyData = android::Equalizer_setParameter(pContext,
                ...
                if (pContext->offload_en ) {
                    ...
                    send_command_to_DSP(pContext->EffectType,
                                        EFFECT_CMD_SET_PARAM,
                                        sizeof(LVM_EQNB_BandDef_t) * FIVEBAND_NUMBANDS,
                                        ActiveParams.pEQNB_BandDefinition);
                }
            }
            if(pContext->EffectType == LVM_VOLUME){
                ALOGD("\tVolume_command cmdCode Case: EFFECT_CMD_SET_PARAM start");
        ...
        case EFFECT_CMD_SET_VOLUME:
        {
            ...
            ALOGD("EFFECT_CMD_SET_VOLUME, left_vol=%d, right_vol=%d", leftVolume, rightVolume);
            ...
            if (pCmdData == NULL || cmdSize != 2 * sizeof(uint32_t) || pReplyData == NULL ||
                    replySize == NULL || *replySize < 2*sizeof(int32_t)) {
                ALOGE("\tLVM_ERROR : Effect_command cmdCode Case: "
                        "EFFECT_CMD_SET_VOLUME: ERROR,pCmdData=%p,cmdSize=%d, pReplyData=%p, replySize=%p",
                        pCmdData, cmdSize, pReplyData, replySize);
                return -EINVAL;
            }

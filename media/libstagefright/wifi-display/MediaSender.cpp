...
MediaSender::MediaSender(
    ...
    : mNetSession(netSession),
      ...
      property_get("ro.build.type", value, "false");
      if(!strcmp(value, "userdebug"))
      {
        property_get("media.wfd.dump.enable", value, "false");
    ...
    if (mMode == MODE_TRANSPORT_STREAM) {
        ...
        for (;;) {
            ...
            if (err == OK) {
                ...
                if(info->mIsAudio) {
                   ALOGI("wfd: MediaSender::queueAccessUnit Audio Packet size: %zu, time: %lld",tsPackets->size(), (long long)nowUs);
                } else {
                   ALOGI("wfd: MediaSender::queueAccessUnit Video Packet size: %zu, time: %lld",tsPackets->size(), (long long)nowUs);
                }

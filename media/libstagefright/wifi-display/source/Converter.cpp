...
status_t Converter::initEncoder() {
    ...
    ALOGI("using audio bitrate of %d bps", audioBitrate);
    ...
    if (isAudio) {
        ...
        {
            videoBitrate = GetInt32Property("media.wfd.video-bitrate", 5000000);
            ALOGI("wfd using 1920x1080 video bitrate of %d bps", videoBitrate);
        }
        else if(width == 1280 && height == 720)
        {
            videoBitrate = GetInt32Property("media.wfd.video-bitrate", 3000000);
            ALOGI("wfd using 1280x720 video bitrate of %d bps", videoBitrate);
        }
        else if(width == 800 && height == 480)
        {
            videoBitrate = GetInt32Property("media.wfd.video-bitrate", 3000000);
            ALOGI("wfd using 800x480 video bitrate of %d bps", videoBitrate);
        }
        else
        {
            videoBitrate = GetInt32Property("media.wfd.video-bitrate", 3000000);
            ALOGI("wfd default using video bitrate of %d bps", videoBitrate);
        }
        ...
        mOutputFormat->setInt32("frame-rate", 31);

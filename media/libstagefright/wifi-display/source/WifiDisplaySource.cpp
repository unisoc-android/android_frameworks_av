...
const int64_t WifiDisplaySource::kKeepAliveIntervalUs;
...
// Enable all resolutions up to 1920x1080 p30
   ...
   ALOGI("wfd support 1920x1080 p30");
}
...
void WifiDisplaySource::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatKeepAliveTimedOut:
        {
            ALOGI("kWhatKeepAliveTimedOut : %d", mKeepAliveCount);
            if (mKeepAliveCount != 0 && mKeepAliveCount <= kKeepAliveTimeoutCount) {
                ++mKeepAliveCount;
                int32_t sessionID;
                CHECK(msg->findInt32("sessionID", &sessionID));
...
status_t WifiDisplaySource::sendM16(int32_t sessionID) {
    ...
    sp<AMessage> msg = new AMessage(kWhatKeepAliveTimedOut, this);
    msg->setInt32("sessionID", sessionID);
    msg->post(kKeepAliveIntervalUs);
...
status_t WifiDisplaySource::onReceiveM16Response(
    ...
    if (mClientInfo.mPlaybackSession != NULL && mKeepAliveCount != 0) {
...
status_t WifiDisplaySource::onSetupRequest(
    ...
    sp<PlaybackSession> playbackSession =
        new PlaybackSession(
                 String16("wifidisplay-sprd"), mNetSession, notify, mInterfaceAddr, mHDCP, mMediaPath.c_str());
